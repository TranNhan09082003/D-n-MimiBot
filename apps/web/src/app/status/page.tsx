...
import { Server, Users, Radio, Activity, Clock, type LucideIcon } from 'lucide-react';
...
export default async function StatusPage() {
  ...
  const metrics: { label: string; value: string; icon: LucideIcon }[] = res.ok
    ? [
        { label: 'Số server', value: formatNumberVN(res.data.guildCount), icon: Server },
        { label: 'Thành viên tiếp cận', value: formatNumberVN(res.data.reachableUsers), icon: Users },
        { label: 'Phiên nghe nhạc', value: formatNumberVN(res.data.activeVoiceSessions), icon: Radio },
        { label: 'Độ trễ WebSocket', value: res.data.wsPing >= 0 ? `${res.data.wsPing} ms` : '—', icon: Activity },
        { label: 'Thời gian hoạt động', value: formatUptime(res.data.uptimeSeconds), icon: Clock },
      ]
    : [];
            ...
            <dl className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
              {metrics.map((m) => {
                ...
                <div key={m.label} className="rounded-xl border border-white/5 bg-white/5 p-4 text-center">
                  <Icon className="mx-auto mb-2 h-5 w-5 text-mimi-lilac" aria-hidden />
