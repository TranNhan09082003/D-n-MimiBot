...
import { Play, Pause, SkipForward, Square, Volume2, Radio } from 'lucide-react';
  ...
  return (
    <div className="mimi-card p-6">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        {paused ? (
          <ControlButton disabled={busy || !connected} onClick={() => act('resume')} label="Tiếp tục" primary>
            <Play className="h-5 w-5 fill-current" aria-hidden />
          </ControlButton>
        ) : (
          <ControlButton disabled={busy || !connected} onClick={() => act('pause')} label="Tạm dừng" primary>
      ...
      {/* Âm lượng */}
      <div className="mt-5">
        <label htmlFor="volume" className="mb-1 flex items-center gap-1.5 text-xs text-mimi-muted">
...
function ProgressBar({
  ...
  if (isStream) {
    return (
      <p className="flex items-center gap-1.5 text-xs text-mimi-cyan">
        <Radio className="h-3.5 w-3.5" aria-hidden /> Phát trực tiếp
      </p>
    );
  }
...
function ControlButton({
  ...
  return (
    <button
      ...
      className={
        primary
          ? 'grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-r from-mimi-lilac to-mimi-pink text-mimi-night shadow-lg shadow-mimi-lilac/25 transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mimi-cyan'
          : 'grid h-11 w-11 place-items-center rounded-xl border border-white/15 bg-white/5 text-mimi-text transition-colors hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mimi-lilac'
      }
    >
      {children}
    </button>
