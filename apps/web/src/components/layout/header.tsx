        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Điều hướng chính">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm text-mimi-muted transition-colors hover:text-mimi-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mimi-lilac"
            >
              {item.href === '/status' && (
                <span className="h-2 w-2 rounded-full bg-mimi-success" aria-hidden />
              )}
              {item.label}
            </Link>
          ))}
